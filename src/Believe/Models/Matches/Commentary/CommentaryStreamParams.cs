...
namespace Believe.Models.Matches.Commentary;
...
public record class CommentaryStreamParams : ParamsBase
{
    ...
    [SetsRequiredMembers]
    public CommentaryStreamParams (
        CommentaryStreamParams commentaryStreamParams
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + string.Format("/matches/{0}/commentary/stream",
            this.MatchID)
        )
        {
