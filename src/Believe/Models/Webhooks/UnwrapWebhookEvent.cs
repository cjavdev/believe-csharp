...
public record class UnwrapWebhookEvent : ModelBase
{
    ...
    public DateTimeOffset CreatedAt {
        get {
            return Match(matchCompleted: ( x )=>x.CreatedAt,
    ...
    public string EventID {
        get {
            return Match(matchCompleted: ( x )=>x.EventID,
            teamMemberTransferred: ( x )=>x.EventID);
        }
    }

    public UnwrapWebhookEvent (
        MatchCompletedWebhookEvent value, JsonElement? element = null
    )
    {
        this.Value = value;
    ...
    public UnwrapWebhookEvent (
        TeamMemberTransferredWebhookEvent value, JsonElement? element = null
    )
    {
        this.Value = value;
        this._element = element;
...
/// </summary>
    public bool TryPickMatchCompleted(
        [NotNullWhen(true)] out MatchCompletedWebhookEvent? value
    )
    {
    ...
    )
    {
        switch (this.Value)
        {
            ...
            case TeamMemberTransferredWebhookEvent value:
                ...
                break;
            default:
                throw new BelieveInvalidDataException("Data did not match any variant of UnwrapWebhookEvent");
...
/// </summary>
    ...
    )
    {
        return this.Value switch
        {
            ...
            _ =>throw new BelieveInvalidDataException("Data did not match any variant of UnwrapWebhookEvent")
        } ;
    }
...
sealed class UnwrapWebhookEventConverter : JsonConverter<UnwrapWebhookEvent>
{
    ...
    )
    {
        ...
        try
        {
            var deserialized = JsonSerializer.Deserialize<MatchCompletedWebhookEvent>(element, options);
        ...
        try
        {
            var deserialized = JsonSerializer.Deserialize<TeamMemberTransferredWebhookEvent>(element, options);
            if (deserialized != null) {
                deserialized.Validate();
                return new(deserialized, element);
            }
