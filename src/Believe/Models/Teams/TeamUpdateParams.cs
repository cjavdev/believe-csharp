...
public record class TeamUpdateParams : ParamsBase
{
    ...
    public ApiEnum<string, League>? League {
        get {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNullableClass<ApiEnum<string, League>>(
                "league"
            );
        }
    ...
    public TeamValues? Values {
        get {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNullableClass<TeamValues>(
                "values"
            );
        }
    ...
    [SetsRequiredMembers]
    public TeamUpdateParams (TeamUpdateParams teamUpdateParams) : base(
        teamUpdateParams
    )
    {
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + string.Format("/teams/{0}",
            this.TeamID)
        )
        {
...
public record class TeamUpdateParamsAnnualBudgetGbp : ModelBase
{
    ...
    public TeamUpdateParamsAnnualBudgetGbp (
    ...
    public void Switch(Action<double> @double, Action<string> @string)
    {
        switch (this.Value)
        {
            ...
            case string value:
                ...
                break;
            default:
                throw new BelieveInvalidDataException("Data did not match any variant of TeamUpdateParamsAnnualBudgetGbp");
...
/// </summary>
    ...
    (Func<double, T> @double, Func<string, T> @string)
    {
        return this.Value switch
        {
            ...
            _ =>throw new BelieveInvalidDataException("Data did not match any variant of TeamUpdateParamsAnnualBudgetGbp")
        } ;
    }

    public static implicit operator TeamUpdateParamsAnnualBudgetGbp (
        double value
    )=> new(value) ;
...
sealed class TeamUpdateParamsAnnualBudgetGbpConverter : JsonConverter<TeamUpdateParamsAnnualBudgetGbp?>
{
