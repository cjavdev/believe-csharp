...
{
    public static JsonElement PrintValue(JsonElement value)
    =>value;

    public static JsonElement PrintValue(
    ...
    public static JsonElement PrintValue(
        IReadOnlyDictionary<string, MultipartJsonElement> value
    )
    {
        ...
        {
            ret[item.Key] = PrintValue(item.Value.Json, item.Value.BinaryContents, ref binaryContentCount);
        }
    ...
    )
    {
        ...
        {
            case JsonValueKind.Undefined:case JsonValueKind.Null:case JsonValueKind.Number:case JsonValueKind.True:case JsonValueKind.False:return json;case JsonValueKind.String:return json.TryGetGuid(out var guid) && binaryContent.ContainsKey(guid)
                ? JsonSerializer.SerializeToElement($"[Binary Content {binaryContentCount++}]") : json;case JsonValueKind.Object:{
                var ret = new Dictionary<string, JsonElement>();
