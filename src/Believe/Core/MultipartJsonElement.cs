...
/// </summary>
    internal IReadOnlyDictionary<Guid, BinaryContent> BinaryContents {
        get; init;
    } = FrozenDictionary.ToFrozenDictionary(new Dictionary<Guid, BinaryContent>());

    public static implicit operator MultipartJsonElement (
        JsonElement json
    )=> new() { Json = json } ;
    ...
    =>JsonSerializer.Serialize(FriendlyJsonPrinter.PrintValue(this), ModelBase.ToStringSerializerOptions);

    public static bool DeepEquals(
        MultipartJsonElement a, MultipartJsonElement b
    )
    =>MultipartJsonElement.DeepEqualsInner(
        a.Json, a.BinaryContents,
        b.Json, b.BinaryContents
    );
    ...
    {
        if(jsonA.ValueKind != jsonB.ValueKind) { return false; }

        switch (jsonA.ValueKind)
        {
            case JsonValueKind.Undefined:case JsonValueKind.Null:case JsonValueKind.True:case JsonValueKind.False:return true;case JsonValueKind.Number:return JsonElement.DeepEquals(jsonA, jsonB);case JsonValueKind.String:BinaryContent? aContent = null;

            BinaryContent? bContent = null;

            if (jsonA.TryGetGuid(out var guidA)
                && binaryA.TryGetValue(guidA, out var a))
            { aContent = a; }

            if (jsonB.TryGetGuid(out var guidB)
                && binaryB.TryGetValue(guidB, out var b))
            { bContent = b; }
            ...
            {
                return false;
            }case JsonValueKind.Object:Dictionary<string, JsonElement> propertiesA = new();
            ...
            foreach(var property in propertiesA)
            {
                ...
                if(!MultipartJsonElement.DeepEqualsInner(property.Value, binaryA,
                    b1, binaryB))
                {
                    return false;
                }
            ...
            return true;case JsonValueKind.Array:if(jsonA.GetArrayLength() != jsonB.GetArrayLength())
            {
                return false;
            }
...
/// </summary>
    static readonly ThreadLocal<Dictionary<Guid, BinaryContent>?> CurrentBinaryContents = new(() => null);
    ...
    static readonly ThreadLocal<Dictionary<JsonSerializerOptions, JsonSerializerOptions>> MultipartSerializerOptionsCache = new(() => new());

    static readonly JsonSerializerOptions DefaultMultipartSerializerOptions = MultipartSerializerOptions(new());

    static JsonSerializerOptions MultipartSerializerOptions(
    ...
    (T value, JsonSerializerOptions? options = null)
    {
        var previousBinaryContents = CurrentBinaryContents.Value;
        try
        {
            CurrentBinaryContents.Value = new();
            var element = JsonSerializer.SerializeToElement(value, MultipartSerializerOptions(options));
            return new()
            {
                Json = element,
    ...
    (MultipartJsonElement element, JsonSerializerOptions? options = null)
    {
        var previousBinaryContents = CurrentBinaryContents.Value;
        try
        {
                ...
                case JsonValueKind.Object:
                    foreach (var item in element.EnumerateObject())
                    {
                        SerializeParts(
                            name == ""
                                ? item.Name
                                : string.Format("{0}[{1}]", name, item.Name),
                            item.Value
                        );
                    }
                    return;
                    ...
                    foreach (var arrayItem in element.EnumerateArray())
                    {
                        switch (arrayItem.ValueKind)
                        {
                            ...
                            case JsonValueKind.False:
                                ...
                                break;
                            case JsonValueKind.String:
                                if(arrayItem.TryGetGuid(out var itemGuid) &&
